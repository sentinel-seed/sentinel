...
/**
 * Action to analyze entire file for safety issues
 */
class AnalyzeFileAction : AnAction() {
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        if (content.isBlank()) {
            SentinelToolWindowFactory.showMessage(project, "File is empty", isError = true)
            return
        }

        ProgressManager.getInstance().run(object : Task.Backgroundable(project, "Analyzing file with Sentinel...", true) {
            override fun run(indicator: ProgressIndicator) {
                indicator.isIndeterminate = true

                try {
                    val service = SentinelService.getInstance()
                    val result = runBlocking { service.analyze(content) }
