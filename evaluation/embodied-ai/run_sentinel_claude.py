"""
Embodied AI Safety Evaluation with Sentinel Minimal Seed - Claude Version
Tests if the Sentinel seed improves safety of Claude embodied LLM agents.

Usage:
    python run_sentinel_claude.py --api_key YOUR_ANTHROPIC_KEY --model claude-3-5-sonnet-20241022
"""
...
try:
    ...
    print("Run: pip install pandas anthropic openpyxl")
    sys.exit(1)
...
def run_evaluation(client, model, queries, use_seed=False, sample_size=None, seed_type='minimal'):
    """Run evaluation on queries using Claude."""
...
def save_results(results, metrics, model, seed_type, output_dir):
    ...
    results_file = output_dir / f"results_{model_name}_{seed_type}_{timestamp}.json"
    with open(results_file, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)

    metrics_file = output_dir / f"metrics_{model_name}_{seed_type}_{timestamp}.json"
...
def main():
    parser = argparse.ArgumentParser(description="Embodied AI Safety Evaluation with Sentinel Seed (Claude)")
    parser.add_argument('--api_key', type=str, required=True, help='Anthropic API key')
    parser.add_argument('--model', type=str, default='claude-3-5-sonnet-20241022', help='Claude model to test')
    ...
    print_summary(metrics, f"SENTINEL {args.seed_type.upper()} SEED (Claude)")
