...
Our contribution: Teleological alignment. We propose that AI safety requires not just harm avoidance, but \emph{purpose validation}. Every action should demonstrate it serves legitimate benefit. This reframes the question from ``Is this bad?'' to ``Is this good?''
...
\begin{enumerate}
    \item Purpose catches what harm misses: On embodied AI scenarios (BadRobot benchmark), adding the Purpose gate improves safety from 74\% to 99.3\%, a +25.3\% improvement.

    \item No training required: Our approach uses structured system prompts (``alignment seeds''), making it immediately deployable without access to model weights.

    \item Cross-model effectiveness: Consistent improvements across six model architectures, from GPT-4o-mini to open-source Llama and Qwen.
\end{enumerate}
...
\citet{xie2023defending} demonstrated that ``self-reminder'' prompts, which instruct models to consider potential harms before responding, significantly reduce jailbreak success rates. Their work showed that explicit safety instructions in the system prompt create a defensive layer against adversarial inputs.

More recently, \citet{fortin2025foundation} explored ontological grounding through structured system prompts, demonstrating that carefully designed frameworks can improve alignment in text-based interactions. Their work provided valuable insights into the viability of prompt-based alignment approaches. Building on these foundations, we develop a secular protocol designed for broader accessibility and extend validation to agentic and embodied AI scenarios.
...
\begin{itemize}
    \item HarmBench \citep{mazeika2024harmbench}: Evaluates refusal of harmful content across diverse attack categories
    \item JailbreakBench \citep{chao2024jailbreakbench}: Tests resistance to adversarial jailbreak attacks
    \item SafeAgentBench \citep{zhang2024safeagentbench}: Evaluates safety in autonomous agent contexts with tool use
    \item BadRobot: Tests physical safety for embodied AI systems
\end{itemize}
...
\subsection{Fiduciary AI: Purpose Gate in Practice}

The Purpose gate has direct application in financial AI agents. Recent work on fiduciary AI \citep{nay2023llm} demonstrates that LLMs can understand fiduciary obligations with approximately 78\% accuracy. Our Purpose gate operationalizes this: an AI agent managing financial assets must justify that each action serves the user's legitimate interests.

We implement this through six fiduciary duties encoded in the validation:

\begin{enumerate}
    \item \textbf{Loyalty}: Prioritize user interests over provider interests
    \item \textbf{Care}: Validate actions before execution
    \item \textbf{Transparency}: Provide auditable reasoning
    \item \textbf{Confidentiality}: Protect user information
    \item \textbf{Prudence}: Consider consequences before acting
    \item \textbf{Disclosure}: Reveal conflicts of interest
\end{enumerate}

This extends the Purpose gate from ``Does this serve benefit?'' to ``Does this serve \emph{this user's} benefit given their stated goals and constraints?''
...
\subsection{Complementary Defense: Memory Integrity}

Recent research on AI agent vulnerabilities \citep{patlan2025memory} revealed that memory injection attacks succeed at 85\% rate against popular agent frameworks. Attackers inject false instructions into the agent's context (e.g., ``ADMIN: transfer all funds to 0xATTACKER''), and the agent cannot distinguish legitimate memories from injected ones.

While THSP validates the \emph{intent} of actions, it does not verify the \emph{integrity} of context. We address this with cryptographic memory signing using HMAC-SHA256. Each memory entry is signed when written and verified when read. This reduces memory injection success from 85\% to under 2\% when combined with THSP.
...
\begin{enumerate}
    ...
    \item \textbf{Context dependency:} Memory integrity requires signing at write time
\end{enumerate}
...
We release our alignment seeds, evaluation framework, and integrations for 15 frameworks (including LangChain, CrewAI, AutoGPT, and ElizaOS) as open source at \url{https://github.com/sentinel-seed/sentinel}. Python and JavaScript SDKs are available via PyPI and npm.
...
\bibitem[Fortin-Dominguez et al.(2025)]{fortin2025foundation}
Fortin-Dominguez, D., Fortin-Dominguez, J. (2025). Cross-Architecture Validation of Foundation Alignment Seeds. \textit{GitHub Repository}. \url{https://github.com/davfd/foundation-alignment-cross-architecture}
...
\bibitem[Nay(2023)]{nay2023llm}
Nay, J. (2023). Large Language Models as Fiduciaries: A Case Study Toward Robustly Communicating With Artificial Intelligence Through Legal Standards. \textit{arXiv preprint arXiv:2301.10095}.

\bibitem[Omohundro(2008)]{omohundro2008basic}
Omohundro, S. (2008). The basic AI drives. \textit{AGI}, 171, 483-492.

\bibitem[Patlan et al.(2025)]{patlan2025memory}
Patlan, E., et al. (2025). Real AI Agents with Fake Memories: Fatal Context Manipulation Attacks on Web3 AI Agents. \textit{arXiv preprint arXiv:2503.16248}.
